...
const Register: React.FC = () => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('📦 Respuesta sin parsear:', data)

      const parsed =
        typeof data.body === 'string' ? JSON.parse(data.body) : data.body

      console.log('✅ Registro exitoso:', parsed)

      if (parsed.dni) {
        alert('Usuario registrado correctamente ✅')
        navigate('/') // Redirige a la raíz (login)
      } else {
        console.error('⚠️ Error en la respuesta del servidor:', parsed)
        alert(parsed.error || parsed.message || 'Error desconocido')
      }
    } catch (err: any) {
